...
const handleSearch =  ()=>{
    const applyFilters = () => {
        return recentdata.filter(item => {
            const matchesDurationFilter = durationFilter === '15min' || item.podcastDuration === durationFilter; 
            const typeFilter = selectedType === "All" || item.podcastType === selectedType; 

            return  matchesDurationFilter  && typeFilter;
        });
    };
        ...
        <Fragment>
            ...
            <div className='h-[73%] bg-white overflow-y-auto' style={{
                    ...
                    <div className="flex flex-wrap gap-2 ms-2 md:gap-4 mt-4 md:mt-8">
                        {entrepreneurs.map((filter) => (
                            <p
                                ...
                                className={`text-sm sm:text-lg text-nowrap px-2  sm:px-3 rounded-full py-1 bg-[#F1F1F1] w-auto ${selectedType === filter ? 'linear_gradient' : 'hover:linear_gradient'}`}
                            >
                                {filter}
                            </p>
                        ))}
                ...
                <div className='flex justify-end mb-8'>
                    ...
                    <button className="px-4 py-2 md:px-8 md:py-3 flex-shrink-0 w-auto ms-2 rounded-lg md:rounded-3xl flex items-center relative cursor-pointer linear_gradient my-2 mr-2 md:mr-4 text-white text-sm md:text-[16px]"  onClick={handleSearch} >
                        Apply Filters
                    </button>
                </div>
            </div>
